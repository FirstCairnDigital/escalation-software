...
class PostgreSQLStoreParityTests(unittest.TestCase):
    @staticmethod
    ...
    def test_sqlite_store_public_methods_are_covered_by_postgresql_store(self):
        sqlite_methods = self._public_methods(SQLiteStore)
        postgres_methods = self._public_methods(PostgreSQLStore)

        missing = sorted(sqlite_methods - postgres_methods)
        self.assertFalse(missing, f"PostgreSQLStore missing SQLite public methods: {missing}")
